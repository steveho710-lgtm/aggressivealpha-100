...
let scanProgress   = 0; // tracks how many stocks done
...
async function fetchHistory(ticker) {
  ...
  if (rows.length<20) throw new Error("Too little history");
  return {
...
// ── Scheduler ─────────────────────────────────────────────────────────────────
cron.schedule("0 15 * * 1-5", ()=>{ console.log("⏰ Auto-scan 1AM AEST"); runScan(); }, {timezone:"UTC"});

// ── /api/scan — triggers background scan, returns immediately ─────────────────
app.get("/api/scan", (req, res) => {
  ...
  // Return immediately — browser will poll /api/results
  res.json({success:false,scanning:true,progress:0,message:"Scan started in background. Poll /api/results every 10 seconds."});
});

// ── /api/results — browser polls this while scan runs ────────────────────────
...
app.get("/api/status",(req,res)=>{
  ...
  res.json({status:"ok",stocks:STOCKS.length,scanInProgress,scanProgress,lastScan:cacheTime?cacheTime.toISOString():"Never",cacheAgeHours:age,nextScheduled:"1:00 AM AEST Mon–Fri",dataSource:"Yahoo Finance direct"});
});

app.get("/api/health",(req,res)=>res.json({status:"ok",message:"AggressiveAlpha-100 running!"}));
app.get("*",(req,res)=>res.sendFile(path.join(__dirname,"public","index.html")));

app.listen(PORT,()=>{
  console.log(`\n⚡ AggressiveAlpha-100 on port ${PORT}`);
  console.log(`   ${STOCKS.length} stocks · Yahoo Finance · background scan · polling API\n`);
});
